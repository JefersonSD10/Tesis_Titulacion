...
natural español mediante un servidor MCP que expone 13 herramientas
...
continuación cada hipótesis específica con la evidencia obtenida:
...
\textbf{se valida}. El sistema integrado Electoral-Peru-2026 combina los
...
indicadores de percepción mediática consultables en lenguaje
natural con evidencia trazable al corpus original. La validación contra
\textit{ground truth} humano sobre el dataset de etiquetado (n=100)
...
validación cuantitativa de la capa GraphRAG sobre el \textit{golden set} de
consulta (n=30) confirma la ventaja del sistema sobre un RAG por similitud
semántica ($\Delta=+0.596$ en corrección de respuesta).
...
\textbf{se valida} con métricas de calidad contra \textit{ground truth}
humano. DeepSeek-Chat en modalidad \textit{few-shot} con 4 ejemplos
calibrados produjo validez del schema del 100\% (655/655 documentos
parseables al primer intento) y cobertura del 100\% sobre el catálogo
cerrado de 17 categorías. Evaluado contra el \textit{gold dataset} de 100
documentos anotados por humano, el módulo obtuvo \textbf{F1 macro 0.856} en
categoría temática (exactitud 0.830), \textbf{F1 0.974} en relevancia
geográfica al Perú, \textbf{F1 macro 0.916} en identificación de candidato
protagonista y \textbf{MAE 0.107} con correlación Pearson de 0.892 en el
sentimiento continuo. El único componente con desempeño limitado fue la
extracción multi-etiqueta de actores principales (F1 micro 0.295).
...
operacionalmente y con evidencia indirecta cuantitativa}. El grafo Neo4j
Community con sus diez tipos de nodos y siete tipos de relaciones permitió
consultas que articulan candidatos, temas, medios, fragmentos y períodos en
una sola \textit{query} Cypher, expresividad que las estructuras tabulares
planas no ofrecen. Los conteos del subgrafo del día (723 documentos, 723
fragmentos, 27 temas, 13 categorías) confirman la correcta construcción y la
idempotencia de las cargas verificada en tres corridas consecutivas con
\textit{counts} constantes en Postgres y Neo4j. Como evidencia indirecta del
valor agregado del grafo, el sistema MCP que consulta sobre Postgres + Neo4j
superó al RAG plano (que opera únicamente sobre \textit{chunks} textuales)
en 25 de 30 preguntas del \textit{golden set}, con una ventaja de $+0.596$
en corrección de respuesta: aporte que el grafo de conocimiento contribuye
estructuralmente más allá de la recuperación textual.

\textbf{HE4} —\textit{GraphRAG mejora la relevancia, coherencia y
contextualización de los insights generados}— \textbf{se valida
cuantitativamente}. El sistema GraphRAG implementado como servidor MCP con
13 herramientas tipadas retornó \texttt{structuredContent} válido contra
esquemas Pydantic v2 en el 100\% de las llamadas \textit{end-to-end}, con
latencia mediana entre 5 y 60\,ms para consultas no vectoriales (frente a
los 1--2\,segundos típicos de un sistema GraphRAG basado en generación
dinámica de \textit{queries}). La métrica directa de cumplimiento de la
hipótesis se midió sobre un \textit{golden set} de 30 preguntas anotadas
...
completo de herramientas esperadas en el 83.3\%} de las preguntas y al menos
una herramienta esperada en el \textbf{100\%}. La evaluación de la respuesta
final mediante un juez independiente (DeepSeek-Chat, $T=0.2$) reportó una
\textbf{corrección de respuesta de 0.792 para el MCP frente a 0.196 para el
RAG \textit{baseline}} por similitud semántica ($\Delta=+0.596$), una
\textbf{relevancia de 0.842 vs 0.388} ($\Delta=+0.454$) y una \textbf{
similitud de \textit{embeddings} de 0.721 vs 0.579} ($\Delta=+0.142$). La
ventaja del MCP es más pronunciada en preguntas \texttt{factual}
($\Delta=+0.902$ en corrección) y \texttt{causal} ($\Delta=+0.480$),
confirmando la superioridad estructural del paradigma de acceso a datos
tabulares por herramientas tipadas frente a la recuperación textual por
\textit{embeddings}.
