...
\\
El corpus electoral procesado por el pipeline completo abarca el período del
\textbf{1 de enero al 6 de abril de 2026} (96 días consecutivos, sin errores de
ejecución) y comprende un total de \textbf{40\,974 documentos} provenientes de
siete canales agrupados en las cuatro fuentes del sistema: noticias digitales
(El Comercio, La República, RPP Noticias y Diario Correo), X/Twitter, YouTube y
TikTok. La distribución temporal del corpus se concentra fuertemente en el mes de
marzo, que reúne el 53\% del volumen total como reflejo del pico de campaña
pre-electoral, según se detalla en la Tabla \ref{5:table_vol_mes}.

\begin{table}[h!]
    \caption[Distribución temporal del corpus completo por mes]{Volumen de documentos del corpus completo (40\,974 documentos) por mes de recolección, 1 de enero al 6 de abril de 2026.}
...
La distribución por fuente de datos del corpus completo se ilustra en la
Figura \ref{5:fig7} y se detalla por canal en la Tabla \ref{5:table_fuente_gen}.
X/Twitter es el canal de mayor volumen (37.8\%), seguido por TikTok (16.3\%) y los
portales de noticias, que en conjunto aportan el 36.1\% del corpus.

\begin{figure}[!ht]
    \begin{center}
        \resizebox{0.85\textwidth}{!}{\input{4/figures/fig7_distribucion_fuentes}}
        \caption[Distribución del corpus electoral total por fuente de datos]{Distribución del corpus electoral completo por las cuatro fuentes de datos (noticias digitales, X/Twitter, YouTube y TikTok), mostrando el volumen relativo de cada canal.\\Fuente: Elaboración propia.}
        \label{5:fig7}
    \end{center}
\end{figure}

\begin{table}[h!]
    \caption[Cobertura del corpus completo por canal de origen]{Distribución de los 40\,974 documentos del corpus completo por canal de origen.}
...
de documentos que mencionan al menos a un candidato: \textbf{6\,293 documentos}
...
Finalmente, la variable dependiente del estudio —el sentimiento mediático hacia
cada candidato— se sintetiza en la Tabla \ref{5:table_sent_cand} para los
candidatos con mayor cobertura. El sentimiento promedio es predominantemente
negativo, coherente con el tono crítico de la cobertura electoral peruana
\parencite{pr_alvi2023twitter_election_review}: César Acuña ($-0.335$) y Vladimir
Cerrón ($-0.406$) presentan los valores más negativos, mientras que Marisol Pérez
Tello ($+0.106$) y Carlos Álvarez ($+0.062$) son los únicos con sentimiento
mediático neto positivo.

\begin{table}[h!]
    \caption[Sentimiento mediático promedio por candidato]{Sentimiento mediático promedio (escala $[-1,1]$) y número de menciones de los candidatos con mayor cobertura en el corpus completo.}
    \label{5:table_sent_cand}
    \centering
    \small
    \begin{tabular}{ m{4.8cm}M{2.5cm}M{2.5cm} }
        \specialrule{.1em}{.05em}{.05em}
        \Centering{Candidato} & \Centering{Menciones} & \Centering{Sentimiento} \\
        \specialrule{.1em}{.05em}{.05em}
        Keiko Fujimori        & 1\,071 & $-0.193$ \\
        Rafael López Aliaga   & 813    & $-0.219$ \\
        César Acuña           & 670    & $-0.335$ \\
        Carlos Álvarez        & 384    & $+0.062$ \\
        Vladimir Cerrón       & 376    & $-0.406$ \\
        Marisol Pérez Tello   & 268    & $+0.106$ \\
        \specialrule{.1em}{.05em}{.05em}
    \end{tabular}
    \begin{flushleft}
        \small Fuente: Elaboración propia.
...
dataset de etiquetado con 100 documentos anotados por un anotador humano
más 15 pares de temas marcados como mismo/distinto tema, que habilita el
...
documentos anotados por un anotador humano sobre la totalidad de los campos
del schema \texttt{AnalisisLLM}: categoría temática, relevancia geográfica al
Perú, candidato protagonista, actores principales y sentimiento hacia el
tema. La construcción del dataset incluyó 2 correcciones manuales sobre las
predicciones del LLM (tasa de corrección del 2\%). Los resultados de comparar
...
recupera aproximadamente un tercio de los actores que el anotador humano
marca como tales; este resultado se interpreta como un sub-objetivo del
\textit{prompt} antes que un fallo del modelo, dado que el LLM tiende a
listar solo los actores nombrados explícitamente en el texto mientras que el
anotador humano incorpora actores implícitos por contexto.
...
\begin{table}[h!]
    ...
    \begin{tabular}{ m{6.5cm}m{6cm} }
        ...
        Costo proyectado mensual ($\sim$20k docs, $\sim$655 docs/día $\times$ 30 días)   & \$3.9 USD$^{a}$ \\
    ...
    \end{tabular}
    \begin{flushleft}
        \small Fuente: Elaboración propia.\\[2pt]
        \small $^{a}$\textbf{Nota de costeo}: El costo proyectado mensual de \$3.9~USD se obtiene del costo efectivo medido: \$0.13/día $\times$ 30~días $\approx$ \$3.9~USD, equivalente a $\sim$655~docs/día $\times$ 30~días $\approx$ \textbf{20k~docs/mes}. Este es el \textit{costo efectivo con caché} (85.5\,\% de \textit{cache hit} sobre el \textit{prompt} de extracción DeepSeek). El \textit{costo nominal sin caché}, a la tasa base de \$0.0008/doc, ascendería a $\sim$\$16~USD/mes para el mismo volumen; la caché de prefijo reduce ese valor en $\approx$75\,\%, dando el costo efectivo de \$3.9~USD/mes. Ambas cifras deben distinguirse al comparar con benchmarks externos que reporten costos sin descuento de caché.
    \end{flushleft}
\end{table}
