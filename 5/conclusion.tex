...
del \textit{override} semántico post-LLM detectó \textbf{4\,378 temas
distintos} sobre el corpus completo (40\,974 documentos, 1 ene -- 6 abr 2026).
La \textbf{tasa de reuso} del catálogo ascendió del 13\% en la corrida inicial
de calibración (catálogo vacío) al 70{,}5\% en corridas posteriores,
evidenciando la curva de maduración esperable. El
...
\texttt{filter\_by\_title\_keywords}) redujo de 818 a 655 documentos ($\sim$20\,\%) el volumen pasado al LLM en la corrida del 22 de marzo de 2026. Dado que los documentos descartados por los filtros son en promedio más cortos (titulares y posts sin cuerpo extendido) que los documentos que sí pasan al LLM, el ahorro en tokens de entrada es proporcionalmente mayor que la reducción en número de documentos: la estimación de $\sim$40\,\% en costo de tokens se obtiene de comparar el total de tokens de entrada procesados por el LLM (medido en la corrida de 655 documentos: $\sim$4.2k tokens/doc promedio, Tabla~16) frente al total estimado de haber procesado los 818 documentos originales incluyendo los descartados (que promedian $\sim$1.8k tokens/doc por ser contenido de menor extensión), resultando en una reducción del costo efectivo de tokens superior a la reducción volumétrica.
...
coincidencia exacta de \textit{slug} desciende a 0.380. Por tanto, \textbf{HE2 se valida en el eje de análisis de sentimiento y clasificación, pero no se valida en el eje de detección de tópicos emergentes}: la exactitud semántica obtenida (0.700) se ubica por debajo del umbral declarado (0.80), por lo que este eje se considera no cumplido. El override semántico post-LLM y las fases de consolidación \texttt{consolidate\_day} + \texttt{llm\_dedup} mitigan operativamente esta brecha, fusionando slugs distintos que comparten significado, pero no modifican el resultado de la evaluación cuantitativa. La incorporación de un umbral de evaluación semántica más granular —por ejemplo, mediante ROUGE-L o BERTScore sobre los nombres de tema— queda como trabajo futuro para subsanar esta limitación de medición.
...
expresión. Sobre el corpus completo, el grafo materializó 98\,808 nodos y
192\,367 relaciones, con correspondencia 1:1 exacta entre los 40\,974
documentos de la capa Silver de Postgres y los 40\,974 nodos \texttt{Documento}
(y 40\,974 nodos \texttt{Fragmento}) del grafo, confirmando la trazabilidad
consistente entre ambos almacenes. Las catorce
...
La magnitud de esta ventaja está influida por la proporción de preguntas
\texttt{factual} sobre datos estructurados (encuestas e indicadores
macroeconómicos) que el RAG vectorial no puede resolver por diseño; esta
diferencia, lejos de ser un sesgo del experimento, es precisamente la
capacidad que el sistema propuesto aporta y que la hipótesis busca verificar.
...
El sistema integrado fue evaluado en sus dos planos de salida: (i)
el módulo de etiquetado mostró F1 macro 0.856 en categoría temática
y MAE 0.107 en sentimiento continuo contra etiquetas humanas
(n=100); (ii) la capa GraphRAG mostró una tool selection accuracy de
83.3\%, una corrección de respuesta de 0.792 y una relevancia de
0.842 sobre un golden set de 30 preguntas evaluado por DeepSeek-Chat
como juez independiente. En cuanto a la fidelidad, que mide el grado de
sustento de la respuesta en el contexto recuperado, es decir, la ausencia
de alucinación, y no su corrección factual, el MCP obtuvo 0.854 frente a
0.868 del RAG por similitud vectorial: una diferencia marginal de 0.014,
dentro del ruido esperado del juez, que indica un nivel de sustento
equivalente entre ambos sistemas. El hallazgo central es que el sistema
propuesto mantiene la misma fidelidad que el RAG sin sacrificar corrección:
el RAG es fiel a los fragmentos que recupera, pero esos fragmentos con
frecuencia no contienen el dato preciso (corrección 0.196), mientras que el
MCP, con un sustento equivalente, accede a la base de datos estructurada y
entrega respuestas factualmente correctas (corrección 0.792, cuatro veces
superior). La articulación de ambos planos, etiquetado y consulta, sobre el
grafo Neo4j permite que las consultas en lenguaje natural devuelvan
respuestas sustentadas por evidencia textual del corpus original, cerrando el
ciclo completo del sistema de inteligencia electoral.
...
En cuarto lugar, la selección de los portales de noticias digitales —RPP Noticias, Diario Correo, El Comercio y La República— responde a criterios de diversidad editorial (espectros centro, popular, liberal y progresista, respectivamente), pero no incluye medios especializados en economía o regiones (como Gestión o medios descentralizados), lo que puede sesgar la cobertura temática hacia la agenda política nacional en detrimento de agendas económicas y subnacionales.
