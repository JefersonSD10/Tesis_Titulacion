...
El análisis automatizado del framing mediante LLMs y su representación en el grafo de conocimiento permite cuantificar y comparar los encuadres predominantes en distintas fuentes mediáticas, identificar consistencias y divergencias entre la prensa escrita, Twitter, YouTube y TikTok —antesala de la polarización de la percepción mediática digital cuando esas divergencias se contraponen—, y rastrear cómo los encuadres evolucionan a lo largo del período electoral. Este análisis constituye uno de los aportes metodológicos centrales de la presente investigación, ya que el grafo de conocimiento permite formular consultas estructuradas del tipo «¿qué medios enmarcan al candidato X principalmente en temas de seguridad con polaridad negativa?» que serían imposibles de responder con representaciones tabulares.
...
A diferencia de la \textit{affective polarization} descrita por \textcite{mc_iyengar2019origins} —que reside en la audiencia y se manifiesta como hostilidad emocional entre grupos de votantes—, esta investigación se concentra en la \textbf{polarización de la percepción mediática digital} —esto es, en la percepción que cada fuente o plataforma \emph{proyecta} sobre el candidato a través de su contenido, y no en la que sostiene el votante—: la dimensión del fenómeno que reside en los medios y describe cómo cada fuente o plataforma trata valorativamente a una misma entidad política. Esta noción no presupone una intención editorial deliberada; basta con el posicionamiento valorativo agregado que emerge de la cobertura de cada fuente o plataforma, con independencia de que responda a una línea editorial consciente o a sesgos no intencionados en la selección y el encuadre del contenido. Conviene además precisar que la polarización de la percepción mediática digital no equivale a mera dispersión: que las fuentes difieran entre sí no basta, pues la divergencia podría repartirse de forma difusa sin configurar bandos. La polarización propiamente dicha exige \textit{bimodalidad}, esto es, la coexistencia de dos polos valorativos opuestos sobre la misma entidad. Por ello su medición se descompone en dos planos complementarios: la \textit{magnitud} de la divergencia agregada entre fuentes, que cuantifica el Índice de Cobertura Mediática Comparada (ICMC), y la \textit{estructura} par a par que localiza las fuentes con tratamientos contrapuestos, que captura el Índice de Divergencia Narrativa (IDN); ambos se definen formalmente en la subsección de Indicadores del Discurso Mediático Electoral.
...
El \textbf{Índice de Cobertura Mediática Comparada} (ICMC) cuantifica la magnitud de la polarización de la percepción mediática digital sobre una misma entidad política, midiendo la dispersión del ISN entre las distintas fuentes que la cubren. Se calcula como la desviación estándar del ISN por fuente respecto a su media, mediante la Ecuación \ref{eq:icmc}:

\begin{equation}\label{eq:icmc}
\phantomsection
ICMC_{(e,t)} = \sqrt{\frac{1}{F}\sum_{f=1}^{F}\left(ISN_f(e,t) - \overline{ISN}(e,t)\right)^2}
\end{equation}
\myequations{Índice de Cobertura Mediática Comparada entre fuentes}

Donde:
\begin{conditions}
ISN_f(e,t)          & Índice de Sentimiento Neto de la entidad $e$ en la fuente $f$ durante el período $t$ \\
\overline{ISN}(e,t) & media del ISN de la entidad $e$ entre las $F$ fuentes que la cubren en el período $t$ \\
F                   & número de fuentes que cubren a la entidad $e$ en el período $t$
\end{conditions}

Un ICMC elevado indica que las fuentes tratan a la entidad de forma marcadamente distinta —síntoma de polarización de la percepción mediática digital—, mientras que un valor cercano a cero refleja consenso narrativo entre medios.

El \textbf{Índice de Divergencia Narrativa} (IDN) complementa al ICMC identificando, par a par, las dos fuentes con el tratamiento más opuesto sobre una misma entidad. Se calcula como la diferencia absoluta del ISN entre un par de fuentes, mediante la Ecuación \ref{eq:idn}:

\begin{equation}\label{eq:idn}
\phantomsection
IDN_{(e,t,f_1,f_2)} = \left| ISN_{f_1}(e,t) - ISN_{f_2}(e,t) \right|
\end{equation}
\myequations{Índice de Divergencia Narrativa entre pares de fuentes}

Donde:
\begin{conditions}
ISN_{f_1}(e,t) & Índice de Sentimiento Neto de la entidad $e$ en la fuente $f_1$ durante el período $t$ \\
ISN_{f_2}(e,t) & Índice de Sentimiento Neto de la entidad $e$ en la fuente $f_2$ durante el período $t$
\end{conditions}

Mientras el ICMC resume la magnitud agregada de la divergencia entre todas las fuentes, el IDN localiza los pares con visiones contrapuestas, evidenciando la estructura de la polarización de la percepción mediática digital (por ejemplo, prensa tradicional frente a plataformas de video corto).

El \textbf{Índice de Relevancia Temática} (IRT) mide la importancia relativa de un tema en el corpus en un período dado, calculado como la proporción de documentos asignados a ese tema sobre el total del corpus en ese período. El \textbf{Índice de Velocidad de Emergencia} (IVE) mide la rapidez con la que un tema nuevo gana relevancia en el discurso mediático, calculado como la derivada temporal del IRT. Estos cinco indicadores se articulan en torno a la polarización de la percepción mediática digital: el ISN desagregado por fuente constituye el insumo común a partir del cual el ICMC y el IDN la operacionalizan —el primero cuantifica su magnitud agregada y el segundo su estructura par a par—, mientras que el IRT y el IVE cumplen un rol complementario al caracterizar la agenda temática en la que se inscribe esa cobertura. Monitoreados conjuntamente en tiempo real y representados estructuralmente en el grafo de conocimiento electoral, constituyen el núcleo analítico del sistema de inteligencia electoral propuesto para el caso de las Elecciones Presidenciales Perú 2026.