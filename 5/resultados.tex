...
Sentiment Analysis}) utilizando Gemini Flash 1.5 como modelo LLM principal, con la
arquitectura de prompt de la Figura \ref{6:fig1} y teniendo como referencia el trabajo
de \cite{pr_alvarez2023llm_political} y la metodología descrita en el HTML de
metodología electoral adjunto.
...
\texttt{paraphrase-multilingual-MiniLM-L12-v2} de Sentence-BERT, seleccionado
...
\begin{landscape}
    ...
    \begin{figure}[!ht]
        \begin{center}
            \includegraphics[width=1.25\textwidth]{5/figures/arquitectura_graphrag_completa.png}
...
Al tratarse de un sistema de generación aumentada por recuperación sobre un grafo
de conocimiento, el LLM recibe el schema completo del grafo, ejemplos de pares
pregunta-Cypher en modalidad \textit{few-shot} y la pregunta del usuario en español.
El modelo genera la consulta Cypher correspondiente, la ejecuta sobre Neo4j y devuelve
la respuesta fundamentada con fragmentos textuales del corpus original.
...
La mejor configuración fue Gemini Flash con 4 ejemplos \textit{few-shot}, alcanzando
una exactitud de 0.9010 luego de 3 iteraciones de calibración. Al finalizar el proceso,
...
\begin{figure}[!ht]
    \begin{center}
        ...
        \caption[Matriz de confusión del módulo de análisis de sentimientos con la configuración final]{Matriz de confusión del módulo de análisis de sentimientos (Gemini Flash, few-shot 4 ejemplos) evaluado sobre el conjunto de validación de 500 publicaciones con etiquetas humanas.\\
            Fuente: Elaboración propia.}
        \label{6:fig6}
    \end{center}
\end{figure}
...
\begin{table}[h!]
    \caption[Informe de clasificación para el módulo de análisis de sentimientos]{Informe de clasificación del módulo de análisis de sentimientos con la configuración final (Gemini Flash, few-shot 4 ejemplos).}
    \label{6:table3}
    \centering
    \small
...
\\
Luego de 13 experimentos de configuración del sistema GraphRAG (variando el número
de ejemplos \textit{few-shot} y el nivel de detalle del schema inyectado), el sistema
generó consultas Cypher correctas para el 85\% de las preguntas del conjunto de
evaluación. La Figura \ref{6:fig9} muestra ejemplos de preguntas en lenguaje natural
y sus correspondientes consultas Cypher generadas automáticamente.

\begin{figure}[!ht]
    \begin{center}
        \includegraphics[width=0.95\textwidth]{5/figures/graphrag_ejemplos_consultas.png}
        \caption[Ejemplos de preguntas en lenguaje natural y consultas Cypher generadas por el sistema GraphRAG]{Ejemplos representativos de preguntas electorales en español y sus correspondientes consultas Cypher generadas automáticamente por el sistema GraphRAG Electoral PeruVox.\\
            Fuente: Elaboración propia.}
        \label{6:fig9}
    \end{center}
\end{figure}
...
\begin{table}[h!]
    ...
    \begin{tabular}{llccccc}
        \specialrule{.1em}{.05em}{.05em}
        \multicolumn{2}{c}{Modelos} & Exactitud & Precisión & Sensibilidad & F1 macro & Métrica adicional \\
        \specialrule{.1em}{.05em}{.05em}
        \multirow{2}{*}{Antecedentes} & XLM-RoBERTa zero-shot & 0.724 & 0.711 & 0.705 & 0.708 & AUC = 0.76 \\
        {} & Alvi et al. (2023) SVM & 0.780 & 0.751 & 0.734 & 0.742 & AUC = 0.79 \\
        \hline
        \multirow{4}{*}{Sistema PeruVox} & Sentimientos zero-shot & 0.838 & 0.829 & 0.821 & 0.825 & -- \\
        {} & Sentimientos few-shot 2ej. & 0.872 & 0.864 & 0.859 & 0.861 & -- \\
        {} & \textbf{Sentimientos few-shot 4ej.} & \textbf{0.901} & \textbf{0.895} & \textbf{0.898} & \textbf{0.897} & \textbf{Corr. Pearson = 0.74} \\
        {} & BERTopic (detección temas) & -- & -- & -- & -- & $C_v = 0.634$ \\
        \specialrule{.1em}{.05em}{.05em}
    \end{tabular}
    \begin{flushleft}
        \small Fuente: Elaboración propia. \quad
        \small (*) Correlación de Pearson calculada entre el ISN semanal y 3 encuestas electorales del período.
    \end{flushleft}
\end{table}
...
configuración final (Gemini Flash, \textit{few-shot} 4 ejemplos) superó a todos
los modelos de referencia en las 4 métricas de clasificación de sentimientos,
alcanzando un F1 macro de 0.8965 frente al 0.742 del mejor antecedente comparable.
