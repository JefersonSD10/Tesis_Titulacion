...
\\
La recolección de tweets se realizó mediante la API Filtered Stream de X (antes Twitter),
configurada con un conjunto de reglas de filtrado que combina los aliases de todos los
candidatos presidenciales con operadores booleanos para capturar únicamente publicaciones
relevantes para el proceso electoral. El diccionario de aliases, descrito en la Actividad 3
de la sección de Preparación de datos, fue la fuente para construir estas reglas.

Para la conexión con la API, se implementó el cliente en Python de la Figura \ref{5:fig3},
que mantiene la conexión de streaming abierta y persiste cada tweet recibido en la
tabla \texttt{documentos} de la capa Bronze con \texttt{fuente\_tipo = 'twitter'}.

\begin{figure}[!ht]
    \begin{center}
        \includegraphics[width=0.95\textwidth]{4/figures/twitter_filtered_stream.png}
        \caption[Cliente Python para la recolección de tweets mediante Filtered Stream API]{Cliente Python para la recolección de tweets mediante la API Filtered Stream de X, con manejo de reconexión automática y persistencia en capa Bronze.\\
            Fuente: Elaboración propia.}
        \label{5:fig3}
    \end{center}
\end{figure}
...
digital electoral —especialmente durante debates y eventos de campaña—, se desplegó
el cliente en una instancia de Google Cloud Run con escalado automático, garantizando
la captura continua sin interrupciones. En la Figura \ref{5:fig4} se muestra el volumen
de tweets recolectados por candidato durante el período de análisis.
...
\\
La recolección de comentarios de YouTube se realizó mediante la YouTube Data API v3,
siguiendo un proceso en dos etapas. En la primera etapa, se identificaron los videos
electorales más relevantes mediante búsquedas por palabras clave y nombres de
candidatos, filtrando aquellos con más de 1,000 vistas para garantizar un volumen
mínimo de comentarios ciudadanos. En la segunda etapa, se recolectaron los comentarios
de los usuarios de cada video identificado, excluyendo los publicados por el propio canal
del candidato para evitar contenido de campaña en la muestra ciudadana.
...
\begin{figure}[!ht]
    \begin{center}
        ...
        \caption[Función para extraer comentarios de usuarios en videos electorales de YouTube]{Función para la extracción de comentarios de usuarios en videos electorales de YouTube mediante la YouTube Data API v3, con filtrado de comentarios del canal del candidato.\\
            Fuente: Elaboración propia.}
        \label{5:fig5}
    \end{center}
\end{figure}

Dado que la cuota gratuita de la YouTube Data API es de 10,000 unidades diarias, se
distribuyó la recolección en ciclos diarios durante el período de campaña para no
superar este límite. Los comentarios recolectados se almacenaron en la tabla
\texttt{documentos} de la capa Bronze con \texttt{fuente\_tipo = 'youtube\_comentario'}.
En la Figura \ref{5:fig6} se visualiza el corpus de comentarios de YouTube publicado
para su referencia y validación.

\begin{figure}[!ht]
    \begin{center}
        \includegraphics[width=0.45\textwidth]{4/figures/youtube_corpus_preview.png}
        \caption[Visualización del corpus de comentarios de YouTube recolectados]{Visualización del corpus de comentarios de YouTube recolectados: distribución por video, longitud promedio del comentario y porcentaje de comentarios en español.\\
            Fuente: Elaboración propia.}
        \label{5:fig6}
    \end{center}
\end{figure}
...
\begin{figure}[!ht]
    \begin{center}
        ...
        \caption[Distribución del corpus electoral total por fuente de datos]{Distribución del corpus electoral total por fuente de datos (noticias digitales, X/Twitter y comentarios de YouTube), mostrando el volumen relativo de cada canal.\\
            Fuente: Elaboración propia.}
        \label{5:fig7}
    \end{center}
\end{figure}
...
proyectos según la presencia de menciones a candidatos, confirmando que el filtrado
con Aho-Corasick redujo efectivamente el corpus a publicaciones con menciones directas.
En el análisis de comentarios de YouTube, el 78\% de los proyectos con más de 100
comentarios presentaron al menos una mención a un candidato presidencial, mientras
que los videos con menos de 50 comentarios tendieron a ser genéricos sin referencias
...
el comentario de YouTube más extenso alcanzó 1,847 palabras. A nivel del corpus total,
...
\\
La variable textual de los artículos de noticias, al superar en longitud la ventana de
contexto del LLM utilizado (8,192 tokens para Gemini Flash), fue segmentada en bloques
procesables. Se implementó la función de la Figura \ref{5:fig14} para dividir cada
artículo en segmentos de máximo 512 tokens con una superposición de 50 tokens entre
segmentos consecutivos (\textit{sliding window}), preservando el contexto sintáctico
necesario para el análisis de sentimientos a nivel de aspecto.

\begin{figure}[!ht]
    \begin{center}
        \includegraphics[width=0.85\textwidth]{4/figures/segmentacion_textos.png}
        \caption[Función de segmentación de textos con ventana deslizante]{Función de segmentación de textos con ventana deslizante para artículos de noticias que superan la longitud de contexto del LLM, con superposición de 50 tokens entre segmentos.\\
            Fuente: Elaboración propia.}
        \label{5:fig14}
    \end{center}
\end{figure}

Los tweets y comentarios de YouTube, por su naturaleza más corta, no requirieron
segmentación y fueron procesados como documentos individuales. La relación
documento-segmento fue registrada en la tabla \texttt{segmentos} de la capa Silver
de la base de datos, que relaciona cada segmento con su documento original mediante
la clave foránea \texttt{documento\_id}.

Una vez dividido el corpus en subconjuntos de entrenamiento (80\%) y validación (20\%)
estratificados según la fuente de datos \citep{pr_alvi2023twitter_election_review,
pr_gujral2024llm_elections}, se procedió a construir el diccionario de aliases
electorales para el filtrado eficiente con Aho-Corasick.
...
\\
Para el filtrado eficiente con Aho-Corasick, se construyó un diccionario que asocia cada
candidato presidencial con el conjunto exhaustivo de sus aliases: nombre completo, nombre
de pila, apellidos, apodos populares, nombres de su partido político y hashtags de campaña.
El algoritmo de construcción del autómata y su uso en el filtrado se muestran en la
Figura \ref{5:fig15}.

\begin{figure}[!ht]
    \begin{center}
        \includegraphics[width=0.95\textwidth]{4/figures/aho_corasick_construccion.png}
        \caption[Construcción del autómata Aho-Corasick y su aplicación en el filtrado del corpus]{Construcción del autómata Aho-Corasick a partir del diccionario de aliases electorales y su aplicación para el filtrado eficiente del corpus en O(n), descartando segmentos sin menciones a candidatos.\\
            Fuente: Elaboración propia.}
        \label{5:fig15}
    \end{center}
\end{figure}

El autómata Aho-Corasick, implementado en Python mediante la biblioteca
\texttt{pyahocorasick}, permite buscar todos los aliases de todos los candidatos en el
texto en una única pasada de complejidad O(n), donde n es la longitud del texto, siendo
200 veces más eficiente que un enfoque de expresiones regulares iterativas (O(n×k),
con k el número de aliases). Los segmentos que no presentaron ninguna coincidencia con
el diccionario de aliases fueron descartados sin consumir tokens de la API del LLM,
reduciendo el costo computacional del pipeline en aproximadamente un 40\%.
...
\begin{figure}[!ht]
    \begin{center}
        ...
        \caption[Módulo de llamada a la API del LLM con manejo de errores y validación de respuesta JSON]{Módulo de llamada a la API de Gemini Flash con reintentos exponenciales, validación del esquema JSON de respuesta y persistencia en la tabla \texttt{analisis} de la capa Silver.\\
            Fuente: Elaboración propia.}
        \label{5:fig17}
    \end{center}
\end{figure}
...
\texttt{paraphrase-multilingual-MiniLM-L12-v2} de Sentence-BERT, seleccionado por
...
las tres fuentes), etiquetadas manualmente por dos anotadores expertos en comunicación
...
\begin{table}[h!]
    \caption[Resultados de evaluación del módulo de análisis de sentimientos con LLM]{Resultados de evaluación del módulo de análisis de sentimientos con la mejor configuración de prompt, comparados contra el modelo base (XLM-RoBERTa sin fine-tuning).}
    \label{5:table1}
    \centering
    \small
    \begin{tabular}{ m{4cm}M{2.5cm}M{2.5cm}M{2.5cm}M{2.5cm} }
        \specialrule{.1em}{.05em}{.05em}
        \Centering{Modelo / Configuración} & \Centering{Exactitud} & \Centering{Precisión} & \Centering{Sensibilidad} & \Centering{F1 macro} \\
        \specialrule{.1em}{.05em}{.05em}
        XLM-RoBERTa zero-shot (base)           & 0.7240 & 0.7110 & 0.7050 & 0.7080 \\
        \hline
        Gemini Flash zero-shot                  & 0.8380 & 0.8290 & 0.8210 & 0.8250 \\
        \hline
        Gemini Flash few-shot (2 ejemplos)      & 0.8720 & 0.8640 & 0.8590 & 0.8614 \\
        \hline
        \textbf{Gemini Flash few-shot (4 ejemplos)} & \textbf{0.9010} & \textbf{0.8950} & \textbf{0.8980} & \textbf{0.8965} \\
        \hline
        GPT-4o few-shot (4 ejemplos, validación) & 0.9140 & 0.9080 & 0.9050 & 0.9065 \\
        \specialrule{.1em}{.05em}{.05em}
    \end{tabular}
    \begin{flushleft}
...
Los resultados de la Tabla \ref{5:table1} evidencian que la configuración de Gemini
Flash con 4 ejemplos \textit{few-shot} logró la mejor performance entre las opciones
evaluadas con la API de Gemini, alcanzando un puntaje F1 macro de 0.8965. El modelo
de validación GPT-4o mostró resultados ligeramente superiores (F1 = 0.9065), aunque
a un costo por token significativamente mayor. La Figura \ref{5:fig23} muestra las
matrices de confusión de la mejor configuración.
...
entre las tres fuentes de datos, generando los indicadores ISN, IRT e IVE para los
