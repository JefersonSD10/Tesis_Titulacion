...
Posteriormente, cada antecedente se sintetizó en una hoja de cálculo de trabajo
interna del equipo, donde se contrastaron sus objetivos, metodologías, modelos
empleados y métricas reportadas; la síntesis cualitativa de ese ejercicio se
expone en la sección de antecedentes del Capítulo~II.
...
El corpus electoral procesado en la corrida de referencia del 22 de marzo de 2026
quedó conformado por \textbf{655 documentos válidos} tras la cascada de filtros
pre-LLM, distribuidos por fuente según los conteos absolutos reportados en la
Tabla \ref{5:table_conteos_fuente}. La proporción relativa derivada de estos
conteos se ilustra gráficamente en la Figura \ref{5:fig7}.

\begin{table}[h!]
    \caption[Conteos absolutos del corpus electoral por fuente de datos]{Conteos absolutos del corpus electoral procesado por fuente de datos en la corrida del 22 de marzo de 2026 (n=655 documentos válidos tras la cascada de filtros pre-LLM), y porcentaje relativo asociado a cada fuente.}
    \label{5:table_conteos_fuente}
    \centering
    \small
    \begin{tabular}{l c c}
        \specialrule{.1em}{.05em}{.05em}
        \textbf{Fuente} & \textbf{Documentos (N)} & \textbf{Porcentaje} \\
        \specialrule{.1em}{.05em}{.05em}
        X/Twitter            & 341 & 52\,\% \\
        \hline
        Noticias digitales   & 118 & 18\,\% \\
        \hline
        TikTok               & 118 & 18\,\% \\
        \hline
        YouTube              &  78 & 12\,\% \\
    ...
    \end{tabular}
    \begin{flushleft}
        \small Fuente: Elaboración propia, a partir de la vista \texttt{gold\_cobertura\_medios} de Postgres.
    \end{flushleft}
\end{table}
...
\begin{figure}[!ht]
    \begin{center}
        ...
        \caption[Distribución del corpus electoral total por fuente de datos]{Distribución relativa (\%) del corpus electoral por las cuatro fuentes de datos (noticias digitales, X/Twitter, YouTube y TikTok), derivada de los conteos absolutos de la Tabla~\ref{5:table_conteos_fuente}.\\Fuente: Elaboración propia.}
        \label{5:fig7}
    \end{center}
\end{figure}
...
dataset de etiquetado con 100 documentos anotados de manera asistida por
dos evaluadores humanos del equipo de investigación, quienes revisaron de
forma conjunta cada documento y resolvieron por consenso las discrepancias
encontradas, más 15 pares de temas marcados como mismo/distinto tema, que habilita el
cálculo de exactitud, F1 macro/micro, MAE/RMSE para el sentimiento continuo
y precision/recall sobre la deduplicación de temas; y (ii) un dataset de
consulta con 30 preguntas en lenguaje natural anotadas con su
\textit{ground truth} de herramientas esperadas y respuesta de referencia,
que permite contrastar el sistema GraphRAG contra un \textit{baseline} RAG
por similitud semántica, conforme al marco de métricas declarado en la
sección \ref{4:metricas-resultados} del capítulo de metodología.
...
documentos cuyo etiquetado fue realizado mediante un protocolo de
\textbf{validación asistida por dos evaluadores humanos} del equipo de
investigación. Ambos evaluadores revisaron de forma conjunta cada documento
sobre la totalidad de los campos del schema \texttt{AnalisisLLM}
—categoría temática, relevancia geográfica al Perú, candidato protagonista,
actores principales y sentimiento hacia el tema— y resolvieron por consenso
las discrepancias detectadas entre la predicción inicial del LLM y el
criterio humano, registrándose 2 correcciones manuales (tasa de corrección
del 2\%). Dado que el protocolo se basó en consenso colaborativo y no en
anotación independiente paralela, no se reporta un coeficiente
$\kappa$ de Cohen calculado sobre etiquetas independientes; esta limitación
de validez interna se discute en el capítulo de conclusiones. Los resultados de comparar
las predicciones de DeepSeek-Chat contra estas etiquetas humanas se reportan
en la Tabla \ref{5:table_label_quality}.
...
\begin{table}[h!]
    ...
    \begin{tabular}{ m{4.2cm}M{1.5cm}M{1.8cm}M{2.0cm}M{2.5cm} }
        ...
        Actores principales (\textit{multi}) & --   & 0.444 & 0.295  & P 0.218 / R 0.457        \\
...
LLM a moderar el sentimiento negativo. El punto débil identificado fue la
extracción de \textbf{actores principales} (F1 micro 0.295), donde el LLM
recupera aproximadamente un tercio de los actores que los evaluadores
humanos marcan como tales; este resultado se interpreta como un sub-objetivo
del \textit{prompt} antes que un fallo del modelo, dado que el LLM tiende a
listar solo los actores nombrados explícitamente en el texto mientras que
los evaluadores humanos incorporan actores implícitos por contexto.
...
El sistema detectó \textbf{27 temas únicos} en el corpus de 655 documentos
...
subconjunto de 15 pares marcados como mismo o distinto tema. Los resultados
se reportan en la Tabla \ref{5:table_topic_quality}.

\begin{table}[h!]
    \caption[Métricas de calidad del módulo de detección y deduplicación de temas]{Métricas de calidad del módulo de detección dinámica de temas y de la fase de \textit{consolidate} + \texttt{llm\_dedup} contra anotaciones humanas. La \textit{exact match} compara el slug exacto del tema; la \textit{lenient} acepta coincidencias por substring; la semántica mide solapamiento de \textit{tokens} normalizados. \textbf{Advertencia estadística:} las métricas de deduplicación se calculan sobre n=15 pares —tamaño muestral en el que cada caso impacta cerca de 50 puntos porcentuales en los indicadores—; los valores reportados (0,500 / 0,500 / 0,500) son \textit{referenciales} y \textbf{no deben interpretarse con el mismo rigor estadístico que las métricas del módulo ABSA evaluadas sobre n=100} (Tabla~\ref{5:table_label_quality}), debido al intervalo de confianza considerablemente amplio asociado a una muestra de este tamaño.}
    ...
    \begin{tabular}{ m{4.8cm}M{2.2cm}M{2.2cm}M{2.2cm} }
        ...
        \specialrule{.1em}{.05em}{.05em}
        Asignación \texttt{tema\_emergente} (n=100) & 0.380 & 0.410 & \textbf{0.700} \\
        \specialrule{.05em}{.05em}{.05em}
         & Precisión & Sensibilidad & F1 \\
        \specialrule{.05em}{.05em}{.05em}
        Deduplicación de temas (n=15)               & 0.500 & 0.500          & 0.500 \\
        \specialrule{.1em}{.05em}{.05em}
    \end{tabular}
    \begin{flushleft}
        \small Fuente: Elaboración propia.
...
exactitud \textit{exact match} (0.380) y la \textbf{exactitud semántica}
(0.700), debido a que el LLM tiende a proponer slugs ligeramente distintos
...
Las métricas de deduplicación se reportan sobre una muestra pequeña: 2 pares
marcados como mismo tema por el sistema (de los cuales 1 es correcto y 1 es
incorrecto) más 13 pares sospechosos no fusionados por el sistema (de los
cuales 1 era un falso negativo). Esta limitación de tamaño muestral —cada
caso pesa $\sim 50$ puntos porcentuales sobre los indicadores— se reconoce
explícitamente y se documenta como restricción de la evaluación.
...
controlado, coherente con el carácter \textbf{cuantitativo} de la
investigación declarado en el capítulo de metodología:
se manipuló la variable independiente
...
%% La relevancia baja en \texttt{edge\_case} en ambos sistemas se explica por el comportamiento de rechazo informativo ante preguntas sobre datos inexistentes, que el juez interpreta como poco relevante.
\begin{table}[h!]
    \caption[Métricas por tipo de pregunta en el golden set MCP vs RAG]{Métricas de corrección, relevancia y similitud de \textit{embeddings} desglosadas por los cuatro tipos de pregunta del \textit{golden set}, expresadas como MCP / RAG / $\Delta$.}
...
\begin{table}[h!]
    \caption[Métricas operacionales del sistema Electoral-Peru-2026 integrado]{Métricas operacionales medidas en la corrida \textit{end-to-end} del pipeline sobre el corpus electoral del 22 de marzo de 2026.}
    \label{5:table7}
    \centering
    \small
...
\\
\noindent\textbf{Nota ética sobre anonimización de actores.} Dado que la prueba piloto se ejecutó sobre datos reales del proceso electoral peruano 2026 —período en el que las candidaturas y los medios involucrados se encuentran activos en campaña— y que la finalidad del presente trabajo es de carácter académico y metodológico, los nombres específicos de los candidatos y de los medios de comunicación que aparecen como salida del sistema se reemplazan en el texto, las figuras y los ejemplos por identificadores anonimizados (\textit{Candidato~A}, \textit{Candidato~B}, \dots, \textit{Medio~1}, \textit{Medio~2}, \dots). Esta decisión obedece a dos consideraciones: (i)~evitar que los indicadores publicados puedan ser interpretados como un juicio de valor sobre actores políticos individuales fuera del contexto técnico de la evaluación; y (ii)~prevenir su uso descontextualizado en el debate público durante el proceso electoral. La correspondencia entre identificadores anonimizados y actores reales se mantiene en los archivos internos del repositorio del proyecto y puede ser puesta a disposición del jurado evaluador bajo acuerdo de confidencialidad para fines de verificación.

\noindent Durante esta actividad se materializaron los flujos de la arquitectura del
