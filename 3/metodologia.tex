...
El presente estudio se define como una \textbf{investigación de tipo cuantitativo}, siguiendo la taxonomía propuesta por \cite{bk_hernandez2014metodologia} en \citetitle{bk_hernandez2014metodologia}, dado que la validación del sistema se sustenta en la recolección sistemática de mediciones numéricas y en el contraste empírico de las hipótesis mediante indicadores objetivos: precisión en la clasificación del sentimiento, coherencia semántica en la detección de temas, y corrección, fidelidad y relevancia de las respuestas generadas por el componente GraphRAG sobre el ecosistema digital electoral peruano. La configuración de la arquitectura computacional —que integra un enfoque \textbf{GraphRAG Híbrido} (bases de datos vectoriales con \textit{pgvector} y de grafos con \textit{Neo4j}), la orquestación de múltiples Modelos Fundacionales como DeepSeek y la exposición de herramientas analíticas vía el \textbf{Model Context Protocol (MCP)}— se evalúa frente a un sistema de referencia (\textit{baseline} RAG por similitud vectorial) sobre un mismo corpus y un mismo \textit{golden set} de preguntas, con el propósito de cuantificar diferencias de desempeño y verificar el cumplimiento de los umbrales declarados en las hipótesis.
...
Según lo explicado en el Capítulo II del presente trabajo, en el ámbito de los sistemas
de analítica de negocio, la Minería de Datos y Big Data, las tres metodologías de
referencia más difundidas en la literatura son SEMMA, CRISP-DM y KDD. Para elegir la
...
De las tres metodologías señaladas (SEMMA, CRISP-DM y KDD), la mayor parte de los antecedentes analizados siguen secuencias de actividades enmarcadas en CRISP-DM, abarcando etapas como en entendimiento del negocio, la recuperación de la información y el procesamiento los datos, la creación del modelo y la evaluación de los resultados. Asimismo, las siguientes características identificadas en los antecedentes influyeron en la selección de la metodología CRISP-DM para el presente estudio:
...
Tras evaluar el desempeño de cada módulo del sistema, se procedió a tomar los resultados y realizar una comparativa respecto a los antecedentes analizados, llevando a cabo una prueba de demostración, en la cual el sistema completo operó en tiempo real durante una semana de campaña electoral. Los especificaciones de esta prueba y sus resultados se presentan en el Capítulo V.
...
—DeepSeek-Chat como juez con temperatura $0.2$ y respuesta forzada a JSON— para evitar
...
Desarrollo del avance de tesis (\textit{valorización académica}) & 900 & S/.30,00 & S/.27\,000,00 \\
\hline
\multicolumn{4}{l}{\textbf{Servicios generales}} \\
Internet y electricidad (5 meses) & 120 & S/.85,00 & S/.680,00 \\
\specialrule{.1em}{.05em}{.05em}
\textbf{Total (incluye valorización académica)} & & & \textbf{S/.41\,880,00} \\
\specialrule{.1em}{.05em}{.05em}
\end{tabular}
\begin{flushleft}
\small Fuente: Elaboración propia. \emph{Nota:} la valorización académica de los recursos humanos es una estimación referencial calculada a una tarifa de S/.\,30,00 por hora (equivalente al salario promedio de un analista \textit{junior} en Lima) sobre las 900 horas dedicadas por el equipo; no constituye un desembolso monetario directo de los autores, sino una valoración del esfuerzo académico empleado.
\end{flushleft}
\end{table}
